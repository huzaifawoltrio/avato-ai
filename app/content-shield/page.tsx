...
export default function ContentShield() {
  ...
  const handleScan = async () => {
    ...
    setTimeout(() => {
      ...
      const newHistoryItem: ScanResult = {
        ...
        status: newResults.plagiarism > 20 ? 'flagged' : newResults.plagiarism > 10 ? 'warning' : 'clean',
      };
  ...
  const getStatusIcon = (status: string) => {
    switch (status) {
      case 'clean': return <CheckCircle className="w-5 h-5 text-green-400" />;
      case 'warning': return <AlertTriangle className="w-5 h-5 text-yellow-400" />;
      case 'flagged': return <XCircle className="w-5 h-5 text-red-400" />;
  ...
  const getScoreColor = (score: number, reverse = false) => {
    if (reverse) {
      return score < 10 ? 'text-green-400' : score < 20 ? 'text-yellow-400' : 'text-red-400';
    }
    ...
    <div className="space-y-8">
      ...
      >
        <div className="flex items-center justify-center space-x-3">
          ...
          <h1 className="text-3xl font-bold text-white">Smart Content Shield™</h1>
        </div>
        <p className="text-gray-400 max-w-2xl mx-auto">
          Scan your content for originality, plagiarism, and SEO optimization. Get AI-powered suggestions to improve your content quality.
        </p>
      </motion.div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <Card className="lg:col-span-1">
          <h2 className="text-xl font-semibold text-white mb-4">Content Input</h2>
          <div className="space-y-4">
            ...
            <div className="border-2 border-dashed border-gray-600 rounded-lg p-6 text-center hover:border-purple-500 transition-colors cursor-pointer">
              ...
              <p className="text-xs text-gray-500 mt-1">Supports .txt, .docx files</p>
            </div>
        ...
        <Card className="lg:col-span-2">
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-3 gap-4">
                <div className="text-center p-4 bg-gray-900/50 rounded-lg">
                  <div className={`text-2xl font-bold ${getScoreColor(scanResults.plagiarism, true)}`}>
                    {scanResults.plagiarism}%
                  </div>
                  <div className="text-sm text-gray-400">Plagiarism</div>
                </div>
                <div className="text-center p-4 bg-gray-900/50 rounded-lg">
                  <div className={`text-2xl font-bold ${getScoreColor(scanResults.seo)}`}>
                    {scanResults.seo}%
                  </div>
                  <div className="text-sm text-gray-400">SEO Score</div>
                </div>
                <div className="text-center p-4 bg-gray-900/50 rounded-lg">
                  <div className={`text-2xl font-bold ${getScoreColor(scanResults.readability)}`}>
          ...
          {!scanResults && !isScanning && (
            <div className="text-center py-8">
              ...
              <p className="text-gray-400">Enter content above and click scan to see results</p>
            </div>
          )}
        </Card>
      </div>
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-gray-700">
                <th className="text-left py-3 px-4 text-gray-300">Content Title</th>
                <th className="text-left py-3 px-4 text-gray-300">Plagiarism %</th>
            ...
            <tbody>
              {history.map((item) => (
                <tr key={item.id} className="border-b border-gray-800 hover:bg-gray-800/30">
                  ...
                  <td className={`py-3 px-4 font-semibold ${getScoreColor(item.plagiarism, true)}`}>
                    {item.plagiarism}%
                  </td>
                  <td className={`py-3 px-4 font-semibold ${getScoreColor(item.seo)}`}>
                    {item.seo}%
                  </td>
                  ...
                  <td className="py-3 px-4">
                    <div className="flex items-center space-x-2">
                      ...
                      <span className="capitalize text-gray-300">{item.status}</span>
                    </div>
                  </td>
                  <td className="py-3 px-4">
